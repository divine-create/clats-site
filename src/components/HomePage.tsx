...
export default function HomePage({
  ...
  return (
    <div className="animate-fadeIn">
      ...
      {/* NEW: 5.5. SEE A LESSON IN ACTION (VIDEO DEMO SECTION) */}
      <section 
        id="lesson-demo-video-section"
        className="py-20 bg-gradient-to-b from-white to-[#fafbff] border-y border-slate-100 relative"
      >
        <div className="absolute top-10 left-10 w-72 h-72 rounded-full bg-teal-400/5 blur-3xl pointer-events-none" />
        <div className="absolute bottom-10 right-10 w-72 h-72 rounded-full bg-purple-400/5 blur-3xl pointer-events-none" />

        <div className="max-w-7xl mx-auto px-6 relative z-10 font-sans">
          
          {/* Header */}
          <div className="text-center max-w-xl mx-auto mb-12 space-y-3">
            <span className="text-[10px] font-mono font-black text-purple-650 bg-purple-500/10 border border-purple-500/15 px-3.5 py-1.5 rounded-full uppercase tracking-wider font-sans inline-block">
              🎥 Video Walkthrough
            </span>
          ...
          <div className="max-w-4xl mx-auto">
            <div className="relative rounded-[2.5rem] overflow-hidden border-4 border-slate-100 shadow-2xl bg-slate-950 p-2 group transition-all duration-300 hover:border-teal-400/30">
              
              {/* Browser mockup header style */}
              <div className="flex items-center justify-between px-6 py-3 border-b border-slate-800 bg-slate-900 rounded-t-[2.2rem] select-none">
                <div className="flex gap-2">
                  <div className="w-3.5 h-3.5 rounded-full bg-[#ff5f56]" />
                  <div className="w-3.5 h-3.5 rounded-full bg-[#ffbd2e]" />
                  <div className="w-3.5 h-3.5 rounded-full bg-[#27c93f]" />
                </div>
                <span className="text-slate-500 text-[10px] font-mono tracking-wider font-semibold">
                  clats-lesson-preview.mp4
                </span>
                <span className="text-slate-500 text-[10px] font-mono tracking-wider font-semibold bg-slate-800 px-2 py-0.5 rounded-md">
                  YouTube Player
                </span>
              </div>

              {/* YouTube IFrame Embed Container */}
              <div className="relative w-full aspect-video rounded-b-[2rem] overflow-hidden bg-slate-900">
                <iframe
                  src="https://www.youtube.com/embed/bHDcP9ao4wk?rel=0&autoplay=0"
                  title="CLATS Lesson Demo Video"
                  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
                  allowFullScreen
            ...
            {/* Quick Incentive Badge below the video */}
            <div className="mt-8 flex flex-col sm:flex-row items-center justify-between p-6 rounded-3xl bg-white border border-slate-100 shadow-lg gap-4">
              <div className="flex gap-3 items-center text-left">
                ...
                <div className="leading-tight font-sans text-left">
                  <span className="text-xs font-black text-slate-950 block">Inspired to start coding?</span>
                  <span className="text-[10px] text-slate-500 font-bold block mt-0.5">Become a Founding Family to get early access to our platform tools!</span>
                </div>
              </div>
              
              <button
                onClick={() => {
                  awardXP(55, 'video_waitlist_clicked');
                  const waitlistSection = document.getElementById('waitlist-section');
                  if (waitlistSection) {
                    waitlistSection.scrollIntoView({ behavior: 'smooth' });
                  }
                }}
                className="w-full sm:w-auto bg-[#2EC4B6] hover:bg-[#25b5a7] text-slate-950 font-black text-xs uppercase tracking-wider px-6 py-3.5 rounded-2xl shadow-md transition-all cursor-pointer border-0 flex items-center justify-center gap-1.5 font-sans"
              >
                Join Waitlist Now
                <ChevronRight className="w-4 h-4" />
              </button>
